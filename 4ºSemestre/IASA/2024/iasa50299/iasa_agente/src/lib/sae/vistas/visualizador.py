...
class Visualizador:
	...
	def pvpix(self, pos_virt):
		...
		"""
		if not isinstance(pos_virt, tuple):
			pos_virt = pos_virt.posicao  # Extract position if it's an EstadoAgente object
		if isinstance(pos_virt, tuple):
			xv, yv = pos_virt  # Unpack the tuple
		else:
			raise TypeError("pos_virt must be a tuple or an EstadoAgente with a posicao attribute")
