...
class Ambiente:
	...
	def limitar_coord(self, coord, coord_min, coord_max):
		return min(coord_max, max(coord_min, coord))
	...
	def mover_agente(self, direccao):
		...
		"""
		# Se direcção definida actualizar direcção
		# senão manter a direcção
		if direccao is not None:
			self._direccao_agente = direccao 
		nova_posicao = self.mover(self._posicao_agente, self._direccao_agente)
	...
	def mover(self, posicao, direccao):
		...
		x_novo = self.limitar_coord(x + round(dx), 0, self._dim_x - 1)
		y_novo = self.limitar_coord(y + round(dy), 0, self._dim_y - 1)
		return x_novo, y_novo
			
	def rodar_agente(self, direccao):
		"""
		Rodar agente com com detecção de colisão
		@param direccao: direccão do movimento
		"""
		self._direccao_agente = direccao
		nova_posicao = self._posicao_agente
		self._colisao = self.verificar_colisao(nova_posicao)
		self._recolha = False
		# Actualizar percepção direccional
		self.detectar_dir()
	...
	def detectar(self, direccao):
		...
		while distancia <= self._dist_max:
			...
			if nova_posicao == posicao: # Limite do ambiente
				break
